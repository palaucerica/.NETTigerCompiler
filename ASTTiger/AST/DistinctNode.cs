...
namespace ASTTiger
{
    public class DistinctNode : RelationalNode
    {
        public DistinctNode(LanguageNode left, LanguageNode right)
            : base(left, right)
        {
        ...
        public override Type GenCode(ILGenerator ilGenerator, TypeBuilder parent, FieldBuilder IsBreakFields, List<FieldBuilder> level, Label endLabel)
        {
            ...
            if (l == typeof(string) || r == typeof(string))
                //Compara los 2 string. Mete en la pila 0 si no son iguales y 1 si lo son 
                ilGenerator.Emit(OpCodes.Call, typeof(String).GetMethod("Equals", new Type[] { typeof(String), typeof(String) }));
